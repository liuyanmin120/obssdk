		...
		obs_data_t *settings)
{
	...
	int x = (int)obs_data_get_int(settings, "x");
	int y = (int)obs_data_get_int(settings, "y");
	int w = (int)obs_data_get_int(settings, "width") ;
	int h = (int)obs_data_get_int(settings, "height");

	if (!w || !h)
	{
		x = (int)obs_data_get_default_int(settings, "x");
		y = (int)obs_data_get_default_int(settings, "y");
		w = (int)obs_data_get_default_int(settings, "width");
		h = (int)obs_data_get_default_int(settings, "height");
	}

	capture->monitor = monitor.id;	
	dc_capture_init(&capture->data, 
			x,
			y,
			w, 
			h, 
...
static void monitor_capture_defaults(obs_data_t *settings)
{
	...
	obs_data_set_default_int(settings, "x", 0);
	obs_data_set_default_int(settings, "y", 0);
...
static void monitor_capture_render(void *data, gs_effect_t *effect,bool is_display)
{
	struct monitor_capture *capture = data;
