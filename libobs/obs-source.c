...
void obs_source_show_force(obs_source_t *source)
{
	if (!obs_source_valid(source, "obs_source_hide_force"))
		return;
	show_source(source);
}

void obs_source_hide_force(obs_source_t *source)
{
	if (!obs_source_valid(source, "obs_source_hide_force"))
		return;
	hide_source(source);
...
const char *obs_source_get_app_encodedId(const obs_source_t *source,void *hwnd)
{
	if (source 
		&& source->info.getAppEncodedId)
	{
		return source->info.getAppEncodedId(hwnd);
	}
	else 
	{
		return NULL;
