...
static void OBSStartStreaming(void *data, calldata_t *params)
{
	...
	// to do start streaming
	g_pBroadcastEngine->engineObserver()->OnSuccessStartPublish();
...
static void OBSStopStreaming(void *data, calldata_t *params)
{
	...
	g_pBroadcastEngine->engineObserver()->OnStopPublish(code, last_error);
	/*QMetaObject::invokeMethod(output->main,
			"StreamingStop", Q_ARG(int, code),
			Q_ARG(QString, arg_last_error));*/
	// to do stopStreaming here
}
...
static void OBSStartRecording(void *data, calldata_t *params)
{
	...
	g_pBroadcastEngine->engineObserver()->OnSuccessStartRecord();
	UNUSED_PARAMETER(params);
}
...
static void OBSStopRecording(void *data, calldata_t *params)
{
	...
	//		Q_ARG(QString, arg_last_error));
	g_pBroadcastEngine->engineObserver()->OnStopRecord(code, last_error);
	UNUSED_PARAMETER(params);
}
...
struct SimpleOutput : BasicOutputHandler {
	...
	SimpleOutput(SourceManager *main_);
...
SimpleOutput::SimpleOutput(SourceManager *main_) : BasicOutputHandler(main_)
{
	const char *encoder = config_get_string(main->Config(), "SimpleOutput",
			"StreamEncoder");
...
bool SimpleOutput::StartStreaming(obs_service_t *service)
{
	...
	if (outputType != type) {
		...
		// extend 
		reconnectStreaming.Disconnect();
		reconnectSuccessStreaming.Disconnect();
		...
		reconnectStreaming.Connect(
			obs_output_get_signal_handler(streamOutput),
			"reconnect", [](void *data, calldata_t *params) 
		{
			int seconds = (int)calldata_int(params, "timeout_sec");
			g_pBroadcastEngine->engineObserver()->OnReconnectStream();

		}, this);

		reconnectSuccessStreaming.Connect(
			obs_output_get_signal_handler(streamOutput),
			"reconnect_success", [](void *data, calldata_t *params)
		{
			g_pBroadcastEngine->engineObserver()->OnReconnectStreamSuccess();
		}, this);
	...
	const char *format)
{
	...
	bool autoRemux = config_get_bool(g_pBroadcastEngine->sourceManger()->Config(), "Video", "AutoRemux");
...
bool SimpleOutput::ConfigureRecording(bool updateReplayBuffer)
{
	...
	if (!dir) {
		blog(LOG_ERROR, "record file: %s is invalid", path);
		//if (main->isVisible())
...
struct AdvancedOutput : BasicOutputHandler {
	...
	AdvancedOutput(SourceManager *main_);
...
static int GetProfilePath(char *path, size_t size, const char *file) 
{
	char profiles_path[512];
	const char *profile = config_get_string(g_pBroadcastEngine->Config(),
		"Basic", "ProfileDir");
	int ret;
...
AdvancedOutput::AdvancedOutput(SourceManager *main_) : BasicOutputHandler(main_)
{
